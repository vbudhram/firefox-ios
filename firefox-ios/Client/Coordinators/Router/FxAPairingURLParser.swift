...
enum FxAPairingURLParser {
    ...
    static func parse(
        ...
        guard scheme == server.scheme?.lowercased(),
              scheme == "https" || isLoopback(host: host),
    ...
    private static func normalized(components: URLComponents, scheme: String?, host: String) -> URL? {
        ...
        // Only rewrite the host when the case actually differs: reassigning it would re-encode an
        // IPv6 literal such as `[::1]`.
        if let currentHost = normalized.host, currentHost != host {
