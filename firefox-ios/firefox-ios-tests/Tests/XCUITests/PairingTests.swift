...
class PairingTests: BaseTestCase {
    ...
    private func waitForModalToClose(timeout: TimeInterval) -> Bool {
        ...
        while Date() < deadline {
            ...
            // Returns false only if the button never showed up within the window, which is the
            // signal being waited on. Each call yields to the runner instead of spinning.
